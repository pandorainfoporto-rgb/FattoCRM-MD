    ...
    <ol className="relative space-y-12 lg:space-y-0 lg:grid lg:grid-cols-4 lg:gap-8">
      {steps.map((step, i) => (
        <li key={i} className="relative">
          ...
          {i < steps.length - 1 && (
            <div className="absolute left-12 top-6 hidden h-px w-full bg-md-cyan/30 lg:block" />
          )}

          <div className="flex items-start gap-4 lg:flex-col lg:gap-0">
            <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full border border-md-cyan bg-md-paper font-display text-lg font-semibold text-md-cyan">
              {step.numero}
            </div>
